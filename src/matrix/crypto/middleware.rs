...
impl MatrixCryptoMiddleware {
    ...
    async fn process_sync_response(
        ...
        // Flush any key requests generated by decryption failures.
        // OlmMachine queues m.room_key_request internally when decrypt_room_event
        // fails with MissingRoomKey; send them immediately rather than waiting
    ...
    async fn process_to_device_events(&mut self, sync_response: &serde_json::Value) {
        ...
        // Log actual OTK counts per algorithm (not just the map length).
        let otk_counts_display: Vec<String> = one_time_keys_counts
            .iter()
        ...
        tracing::info!(
            ...
            otk_counts = %otk_counts_display.join(", "),
